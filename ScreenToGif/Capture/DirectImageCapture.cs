    ...
    internal class DirectImageCapture : BaseCapture
    {
        ...
        private void FallbackCursorCapture(FrameInfo frame)
        {
            //if (_justStarted && (CursorShapeBuffer?.Length ?? 0) == 0)
            {
                ...
                var infoHeader = new Util.Native.BitmapInfoHeader();
                    ...
                    if (cursorInfo.flags == Util.Native.CursorShowing)
                    {
                        ...
                        if (hicon != IntPtr.Zero)
                        {
                            if (Util.Native.GetIconInfo(hicon, out var iconInfo))
                            {
                                ...
                                var bitmap = new Util.Native.Bitmap();
                                var hndl = GCHandle.Alloc(bitmap, GCHandleType.Pinned);
                                var ptrToBitmap = hndl.AddrOfPinnedObject();
                                Util.Native.GetObject(iconInfo.hbmColor, Marshal.SizeOf<Util.Native.Bitmap>(), ptrToBitmap);
                                bitmap = Marshal.PtrToStructure<Util.Native.Bitmap>(ptrToBitmap);
                                hndl.Free();
                                ...
                                var windowDeviceContext = Util.Native.GetWindowDC(IntPtr.Zero);
                                var compatibleBitmap = Util.Native.CreateCompatibleBitmap(windowDeviceContext, Width, Height);

                                Util.Native.GetDIBits(windowDeviceContext, compatibleBitmap, 0, (uint)infoHeader.biHeight, CursorShapeBuffer, ref infoHeader, Util.Native.DibColorMode.DibRgbColors);
