...
namespace ScreenToGif.Cloud.Gfycat
{
    public class Gfycat : IUploader
    {
        ...
        {
            if (!(preset is GfycatPreset gfycatPreset))
                throw new Exception("Gfycat preset is null.");

            if (!await IsAuthorized(gfycatPreset))
                throw new UploadingException("It was not possible to get the authorization to upload to Gfycat.");
        ...
        {
            var create = preset.AskForDetails ? Application.Current.Dispatcher.Invoke(() => UploadDetailsDialog.OkCancel(preset)) : new GfycatCreateRequest(preset);
        ...
        [Obsolete("Maybe switch to using HttpClient, if it's a better solution")]
